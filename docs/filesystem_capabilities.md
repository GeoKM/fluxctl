...
| CBM DOS 1541/1571 | Yes | Yes | Yes | Root only | Root file import, replace, and scratch/delete for `.d64`/`.d71` | Directory import and directory creation are pending. Import infers `.PRG`, `.SEQ`, and `.USR`; unknown suffixes default to PRG. REL extraction follows side-sector indexes and remains block/record padded; REL mutation is pending because side-sector allocation is required. |
