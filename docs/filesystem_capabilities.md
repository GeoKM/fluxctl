...
| CP/M variants | Yes | Yes | Yes for modelled DPBs | Root only | No | CP/M 26-sector 250K extraction is supported. Other CP/M variants can list directory entries, but need their own disk parameter blocks before extraction is enabled. |
...
| CP/M variants | Yes | Yes | Root only | Yes | Yes for CP/M 26-sector 250K files | Files for modelled DPBs | No | No | No | No | No | No | C64 CP/M 2.2 only; more CP/M DPBs pending | Filesystem logical map |
...
- CP/M export is enabled only when Fluxctl has a modelled disk parameter block.
  The CP/M 26-sector 250K DPB is supported; C64/C128 and other generic CP/M
...
- **CP/M**: exports files for modelled disk parameter blocks such as CP/M
  26-sector 250K media. Other CP/M variants still list root entries but reject
