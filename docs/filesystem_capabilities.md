...
| CP/M variants | Yes | Yes | Yes for modelled DPBs | Root only | No | CP/M 26-sector 256K FM extraction is supported. Other CP/M variants can list directory entries, but need their own disk parameter blocks before extraction is enabled. |
...
| CP/M variants | Yes | Yes | Root only | Yes | Yes for CP/M 26-sector 256K FM files | Files for modelled DPBs | No | No | No | No | No | No | C64 CP/M 2.2 only; more CP/M DPBs pending | Filesystem logical map |
...
- CP/M export is enabled only when Fluxctl has a modelled disk parameter block.
  The CP/M 26-sector 256K FM DPB is supported; C64/C128 and other generic CP/M
  layouts still need their own allocation maps.
...
- **CP/M**: exports files for modelled disk parameter blocks such as CP/M
  26-sector 256K FM media. Other CP/M variants still list root entries but reject
  extraction until their DPB and sector translation are implemented.
